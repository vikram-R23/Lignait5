...
const ResumeSkills = () => {
  ...
  const [chatMessages, setChatMessages] = useState([
    { id: 1, sender: 'ai', text: 'Hi Baskar! I can suggest relevant skills based on your target role. What job are you applying for?' }
  ]);
  ...
  useEffect(() => {
    const fetchSkillsData = async () => {
      try {
        console.log("System Ready: Resume Skills module initialized.");
      } catch (err) {
  ...
  const handleChatSubmit = (e) => {
    ...
    setChatInput("");
    setTimeout(() => {
        const newAiMsg = { id: Date.now() + 1, sender: 'ai', text: "Here are some top skills for that position..." };
              ...
              <ul className="space-y-1">
                ...
                <li>
                  ...
                  <button className="w-full flex items-center justify-between p-2.5 rounded-lg text-sm font-medium text-white bg-[#0474C4] shadow-md text-left transition-all">
                    <div className="flex items-center gap-3">
                      <span className="material-symbols-outlined text-white text-[20px]">build</span>
                      Skills
                    </div>
                    <span className="material-symbols-outlined text-[16px] opacity-70">drag_handle</span>
                  </button>
                </li>
                {['Projects', 'Certifications'].map((item, idx) => (
                  <li key={idx}>
                    <button 
                      onClick={() => navigate(`/resume-${item.toLowerCase()}`)}
                      className="w-full flex items-center justify-between p-2.5 rounded-lg text-sm font-medium text-[#D1D5DB] hover:bg-white/5 hover:text-white group text-left transition-all"
                    >
                      <div className="flex items-center gap-3">
                        <span className="material-symbols-outlined text-[#D1D5DB] group-hover:text-white text-[20px]">
                          {item === 'Projects' ? 'folder' : 'verified'}
                        </span>
                        {item}
                      </div>
                      <div className="opacity-0 group-hover:opacity-100 flex gap-1">
                        <span className="material-symbols-outlined text-[16px] text-[#D1D5DB] hover:text-white cursor-pointer">arrow_upward</span>
                        <span className="material-symbols-outlined text-[16px] text-[#D1D5DB] hover:text-white cursor-pointer">arrow_downward</span>
                      </div>
                    </button>
                  </li>
                ))}
          ...
          <div className="flex-1 bg-[#06457F] overflow-y-auto relative scroll-smooth">
            <div className="max-w-2xl mx-auto p-6 pb-24">
              <div className="mb-8 p-4 bg-[#053b6e]/50 border border-[#0474C4]/30 rounded-xl flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm text-white font-medium">Smart Skills Suggestion</p>
                  <p className="text-xs text-[#D1D5DB] mt-1 leading-relaxed">
                    Based on your 'Senior Product Designer' experience, we've suggested these core competencies.
                  </p>
                </div>
              </div>

              <div className="flex items-center justify-between mb-6">
                <h2 className="text-2xl font-bold text-white" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>Skills</h2>
              ...
              <div className="bg-[#1E1E1E] rounded-xl border border-white/10 p-6">
                <div className="flex flex-wrap gap-2">
                  {['Figma', 'Sketch', 'Adobe CC', 'HTML/CSS', 'Prototyping', 'User Research', 'Design Systems', 'Agile Methodology'].map((skill, index) => (
                    <div key={index} className="flex items-center gap-2 bg-[#0474C4]/20 text-white border border-[#0474C4]/30 px-3 py-1.5 rounded-full text-sm font-medium group">
                      {skill}
                      <button className="text-white/40 hover:text-white"><span className="material-symbols-outlined text-[16px]">close</span></button>
                    </div>
                  ))}
                </div>
                <div className="mt-8 border-t border-white/5 pt-6">
                  <label className="block text-xs font-medium text-[#D1D5DB] mb-3 uppercase tracking-wider">Add New Skill</label>
                  <div className="flex gap-2">
                    <input className="flex-1 bg-[#1E1E1E] border border-[#3A3A3A] rounded-lg px-3 py-2 text-sm text-white placeholder-[#9CA3AF] focus:border-[#0474C4] focus:ring-1 focus:ring-[#0474C4] outline-none" placeholder="e.g. Interaction Design" type="text"/>
                    <button className="px-4 py-2 bg-white/10 hover:bg-white/20 text-white rounded-lg text-sm font-medium transition-colors">Add</button>
                  </div>
                </div>
              </div>
                  ...
                  <div className="flex-1 flex flex-col gap-4">
                    ...
                    <div>
                      ...
                      <div className="mb-2">
                        ...
                        <p className="text-[10px] text-slate-600 leading-normal">Developed an end-to-end investment portal used by 50k users.</p>
                      </div>
                    </div>
                  </div>
                </div>
