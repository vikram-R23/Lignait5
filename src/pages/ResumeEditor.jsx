...
const ResumeEditor = () => {
  ...
  const [chatMessages, setChatMessages] = useState([
    { id: 1, sender: 'ai', text: 'Hi Baskar! Need help optimizing your resume content or phrasing your experience?' }
  ]);
  ...
  useEffect(() => {
    const fetchEditorData = async () => {
      try {
        console.log("System Ready: Resume Editor API initialized.");
      } catch (err) {
  ...
  const handleChatSubmit = (e) => {
    ...
    setChatInput("");
    setTimeout(() => {
        const newAiMsg = { id: Date.now() + 1, sender: 'ai', text: "I can suggest stronger action verbs for that section..." };
    ...
    <div className="bg-[#06457F] text-white font-['Inter'] h-screen w-full flex overflow-hidden antialiased relative">
      ...
      <aside className="w-72 flex-shrink-0 flex flex-col border-r border-slate-300 bg-white relative z-50 text-[#0F172A]">
        ...
        <nav className="flex-1 px-4 py-4 flex flex-col gap-2 overflow-y-auto no-scrollbar">
          ...
          {/* Active State for Resume Editor */}
              ...
              <ul className="space-y-1">
                <li>
                  <button onClick={() => navigate('/resume-editor')} className={getSidebarItemClass('/resume-editor')}>
                    ...
                    {/* Always show drag handle for Personal Info if active or default, but making it consistent */}
                    {location.pathname === '/resume-editor' && <span className="material-symbols-outlined text-[16px] opacity-70">drag_handle</span>}
                  </button>
                </li>
                <li>
          ...
          <div className="flex-1 bg-[#06457F] overflow-y-auto relative scroll-smooth">
            <div className="max-w-2xl mx-auto p-6 pb-24">
              ...
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-2xl font-bold text-white" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>Personal Information</h2>
              </div>
              ...
              <div className="max-w-3xl space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="flex flex-col gap-2">
                    ...
                    <input className="bg-[#1E1E1E] border border-[#3A3A3A] rounded-md px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-[#0474C4] focus:ring-1 focus:ring-[#0474C4] transition-all" placeholder="e.g. Jane" type="text" defaultValue="Alex"/>
                  </div>
                  <div className="flex flex-col gap-2">
                    <label className="text-sm font-medium text-[#D1D5DB]">Last Name</label>
                    <input className="bg-[#1E1E1E] border border-[#3A3A3A] rounded-md px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-[#0474C4] focus:ring-1 focus:ring-[#0474C4] transition-all" placeholder="e.g. Doe" type="text" defaultValue="Morgan"/>
                  </div>
                </div>
                
                <div className="flex flex-col gap-2">
                  <label className="text-sm font-medium text-[#D1D5DB]">Professional Headline</label>
                  <input className="bg-[#1E1E1E] border border-[#3A3A3A] rounded-md px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-[#0474C4] focus:ring-1 focus:ring-[#0474C4] transition-all" placeholder="e.g. Software Engineer" type="text" defaultValue="Senior Product Designer"/>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="flex flex-col gap-2">
                    <label className="text-sm font-medium text-[#D1D5DB]">Email Address</label>
                    <input className="bg-[#1E1E1E] border border-[#3A3A3A] rounded-md px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-[#0474C4] focus:ring-1 focus:ring-[#0474C4] transition-all" placeholder="e.g. jane@example.com" type="email" defaultValue="alex.morgan@example.com"/>
                  </div>
                  <div className="flex flex-col gap-2">
                    <label className="text-sm font-medium text-[#D1D5DB]">Phone Number</label>
                    <input className="bg-[#1E1E1E] border border-[#3A3A3A] rounded-md px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-[#0474C4] focus:ring-1 focus:ring-[#0474C4] transition-all" placeholder="e.g. +1 555 000 0000" type="tel" defaultValue="+1 (555) 123-4567"/>
                  </div>
                </div>

                <div className="flex flex-col gap-2">
                  <label className="text-sm font-medium text-[#D1D5DB]">Address / Location</label>
                  <input className="bg-[#1E1E1E] border border-[#3A3A3A] rounded-md px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-[#0474C4] focus:ring-1 focus:ring-[#0474C4] transition-all" placeholder="e.g. New York, NY" type="text" defaultValue="San Francisco, CA"/>
                </div>

                <div className="flex flex-col gap-2">
                  <div className="flex justify-between items-center">
                    <label className="text-sm font-medium text-[#D1D5DB]">Professional Summary</label>
                    <span className="text-xs text-[#6fa6d4]">Recommended: 3-5 sentences</span>
                  </div>
                  <textarea className="bg-[#1E1E1E] border border-[#3A3A3A] rounded-md px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-[#0474C4] focus:ring-1 focus:ring-[#0474C4] resize-none transition-all" placeholder="Briefly describe your professional background..." rows="6" defaultValue="Experienced Product Designer with over 5 years of experience in building user-centric digital products. Proven track record of improving user engagement and streamlining workflows. Skilled in Figma, prototyping, and design systems."></textarea>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
                  <div className="flex flex-col gap-2">
                    <label className="text-sm font-medium text-[#D1D5DB]">LinkedIn Profile <span className="text-gray-500 font-normal">(Optional)</span></label>
                    <input className="bg-[#1E1E1E] border border-[#3A3A3A] rounded-md px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-[#0474C4] focus:ring-1 focus:ring-[#0474C4] transition-all" placeholder="linkedin.com/in/username" type="text"/>
                  </div>
                  <div className="flex flex-col gap-2">
                    <label className="text-sm font-medium text-[#D1D5DB]">Portfolio URL <span className="text-gray-500 font-normal">(Optional)</span></label>
                    <input className="bg-[#1E1E1E] border border-[#3A3A3A] rounded-md px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-[#0474C4] focus:ring-1 focus:ring-[#0474C4] transition-all" placeholder="yourportfolio.com" type="text"/>
                  </div>
                </div>

                <div className="pt-8 flex justify-end pb-12">
                  <button onClick={() => navigate('/resume-experience')} className="bg-[#0474C4] hover:bg-[#0361a3] text-white px-6 py-2 rounded-md font-semibold transition-colors flex items-center gap-2">
                    Next: Experience
