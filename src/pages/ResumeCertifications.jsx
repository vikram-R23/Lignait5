...
const ResumeCertifications = () => {
  ...
  const [chatMessages, setChatMessages] = useState([
    { id: 1, sender: 'ai', text: 'Hi Baskar! Certifications validate your skills. Need help verifying a credential ID?' }
  ]);
  ...
  useEffect(() => {
    const fetchCertData = async () => {
      try {
        console.log("System Ready: Resume Certifications module initialized.");
      } catch (err) {
  ...
  const handleChatSubmit = (e) => {
    ...
    setChatInput("");
    setTimeout(() => {
        const newAiMsg = { id: Date.now() + 1, sender: 'ai', text: "That's a valuable certification! It should go near the top of your skills section..." };
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="hidden md:flex flex-col gap-1 w-48">
              <div className="flex justify-between items-end">
                ...
                <span className="text-xs font-bold text-[#0474C4]">95%</span>
              </div>
              <div className="h-1.5 w-full bg-[#1E1E1E] rounded-full overflow-hidden border border-white/5">
                <div className="h-full bg-[#0474C4] rounded-full" style={{ width: '95%' }}></div>
              </div>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <span className="text-xs text-[#D1D5DB] hidden sm:inline-block">Last saved just now</span>
          ...
          <div className="w-64 flex-shrink-0 bg-[#06457F] border-r border-white/10 flex flex-col">
            <div className="p-4">
              ...
              <ul className="space-y-1">
                ...
                <li>
                  <button onClick={() => navigate('/resume-experience')} className="w-full flex items-center justify-between p-2.5 rounded-lg text-sm font-medium text-[#D1D5DB] hover:bg-white/5 hover:text-white group text-left transition-all">
                    <div className="flex items-center gap-3">
                      <span className="material-symbols-outlined text-[#D1D5DB] group-hover:text-white text-[20px]">work</span>
                      Experience
                    </div>
                  </button>
                </li>
          ...
          <div className="flex-1 bg-[#06457F] overflow-y-auto relative scroll-smooth">
            <div className="max-w-2xl mx-auto p-6 pb-24">
              <div className="mb-8 p-4 bg-[#053b6e]/50 border border-[#0474C4]/30 rounded-xl flex items-start gap-3">
                <span className="material-symbols-outlined text-[#0474C4] shrink-0 mt-0.5">info</span>
                <div>
                  <p className="text-sm text-white font-medium">Certification Tips</p>
                  <p className="text-xs text-[#D1D5DB] mt-1 leading-relaxed">
                    Adding industry-standard certifications can increase your profile visibility by up to 40%. Make sure to include the Credential ID for verification.
                  </p>
                </div>
              </div>

              <div className="flex items-center justify-between mb-6">
                <h2 className="text-2xl font-bold text-white" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>Certifications</h2>
              ...
              <div className="space-y-6">
                <div className="bg-[#1E1E1E] rounded-xl border border-white/10 p-6 relative group/item">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="col-span-2">
                            <label className="block text-xs font-medium text-[#D1D5DB] mb-1.5 uppercase tracking-wider">Certification Name</label>
                            <input className="w-full bg-[#1E1E1E] border border-[#3A3A3A] rounded-lg px-4 py-2.5 text-sm text-white placeholder-[#9CA3AF] focus:border-[#0474C4] focus:ring-1 focus:ring-[#0474C4] outline-none" type="text" defaultValue="AWS Certified Solutions Architect"/>
                        </div>
                        <div className="col-span-2">
                            <label className="block text-xs font-medium text-[#D1D5DB] mb-1.5 uppercase tracking-wider">Issuing Organization</label>
                            <input className="w-full bg-[#1E1E1E] border border-[#3A3A3A] rounded-lg px-4 py-2.5 text-sm text-white placeholder-[#9CA3AF] focus:border-[#0474C4] focus:ring-1 focus:ring-[#0474C4] outline-none" type="text" defaultValue="Amazon Web Services"/>
                        </div>
                        <div>
                            <label className="block text-xs font-medium text-[#D1D5DB] mb-1.5 uppercase tracking-wider">Issue Date</label>
                            <input className="w-full bg-[#1E1E1E] border border-[#3A3A3A] rounded-lg px-4 py-2.5 text-sm text-white placeholder-[#9CA3AF] focus:border-[#0474C4] focus:ring-1 focus:ring-[#0474C4] outline-none" type="text" defaultValue="March 2023"/>
                        </div>
                        <div>
                            <label className="block text-xs font-medium text-[#D1D5DB] mb-1.5 uppercase tracking-wider">Credential ID</label>
                            <input className="w-full bg-[#1E1E1E] border border-[#3A3A3A] rounded-lg px-4 py-2.5 text-sm text-white placeholder-[#9CA3AF] focus:border-[#0474C4] focus:ring-1 focus:ring-[#0474C4] outline-none" type="text" defaultValue="AWS-123456789"/>
                        </div>
                    </div>
                </div>
                
                <button className="w-full py-4 border-2 border-dashed border-white/10 rounded-xl text-sm font-medium text-[#D1D5DB] hover:text-white hover:border-[#0474C4]/50 hover:bg-white/5 transition-all flex items-center justify-center gap-2">
                    <span className="material-symbols-outlined text-[20px]">add_circle</span>
                    Add Another Certification
                </button>
              </div>

              {/* NAVIGATION LOGIC - FINAL STEP */}
              <div className="pt-8 flex justify-end pb-12">
               <button 
                  onClick={() => navigate('/resume-score')} 
                  className="bg-green-600 hover:bg-green-700 text-white px-6 py-2 rounded-md font-bold transition-colors flex items-center gap-2 shadow-lg"
                >
                  Finish & Save
              ...
              <div className="a4-paper shadow-2xl relative">
                <div className="p-8 h-full flex flex-col">
                  ...
                  <div className="flex-1 flex flex-col gap-4">
                    ...
                    {/* Certifications Highlighted in Preview */}
                    <div className="bg-blue-50/50 -mx-2 px-2 py-1 rounded">
                        <h2 className="text-xs font-bold text-slate-800 uppercase border-b border-slate-200 pb-1 mb-2">Certifications</h2>
                        <div className="flex justify-between items-baseline mb-1">
                            <h3 className="font-bold text-slate-900 text-[10px]">AWS Certified Solutions Architect</h3>
                            <span className="text-[9px] text-slate-500">Mar 2023</span>
                        </div>
                        <p className="text-[9px] text-slate-600">Amazon Web Services • ID: AWS-123456789</p>
                    </div>
